...
export function AuthGate({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center bg-[#1e1e1e]/95 backdrop-blur-sm"
      role="dialog"
      aria-modal="true"
    ...
    >
      <form
        onSubmit={onSubmit}
        className="w-full max-w-lg rounded-md border border-white/10 bg-[#252526] p-6 shadow-2xl"
      >
        <div className="mb-4 flex items-center gap-3">
          ...
          <div>
            <h2 id="auth-gate-title" className="text-lg font-semibold text-white">
        ...
        <label className="mb-2 block text-xs tracking-wide text-gray-400 uppercase" htmlFor="token">
          Bearer token
        </label>
        <input
          id="token"
        ...
        <div className="mt-5 flex items-center justify-end gap-2">
          <button
            ...
            className="inline-flex items-center gap-2 rounded bg-sky-600 px-4 py-1.5 text-sm font-medium text-white hover:bg-sky-500 disabled:opacity-60"
          >
            {state === "probing" ? (
              <>
        ...
        <p className="mt-4 border-t border-white/5 pt-3 text-[11px] leading-snug text-gray-500">
          ...
          <code className="font-mono text-[11px]">?token=…</code> query param on every WebSocket.
          Clear it by running{" "}
