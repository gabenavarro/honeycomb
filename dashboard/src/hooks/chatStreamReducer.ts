/** chatStreamReducer — pure reducer extracted from useChatStream (M37 follow-up).
 *
 * Extracted so the module-level chatStreamStore can import applyEvent
 * without pulling in React. Both the store and the tests import from here.
 */
import type { ChatCliEvent, ChatTurn, ChatBlock, StreamEventInner } from "../components/chat/types";

export function applyEvent(prev: ChatTurn[], event: ChatCliEvent): ChatTurn[] {
